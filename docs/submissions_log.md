...
## #9 — 2026-07-05 06:40:11 UTC — ✅ 6/6 feasible, P3 −56%

| 인스턴스 | #8 | #9 | 비고 |
|---|---:|---:|---|
| P1 | 43,300 | **43,300** | narrow 불변 |
| P2 | 84,764 | **84,764** | narrow 불변 |
| P3 | 24,515,298 | **10,793,058** | **−56.0%** (safe 컬럼 최적화 A1/A2/A3 적중) |
| P4 | 10,761,704 | **10,761,704** | wide 불변 |
| P5 | 25,903,553 | **25,903,553** | wide 불변 |
| P6 | 61,359,812 | **61,359,812** | wide 불변 |

총합 **108,946,191** (#8 122.7M 대비 −11.2%). P4/P5/P6가 3제출 연속 동일 = wide가 서버에서
결정적으로 재현됨(단일 spec + LS라 실행 경로 고정).

## ★ #10 준비분 — wide 탐색폭 확대 + safe 16-spec

**Wide 분석(서버-시뮬, mp 차단, TL=60)**: wide는 첫 spec(seed0/exit)이 구축창 28.2s 전부를
소비 → 멀티스타트 미작동(서버 값 3회 동일의 원인). P6-like 성분: obj1(tardiness) ~96-97% 지배.

**실험 (대형 train 7개: prob_23/24/26/29/30/35/40, paired)**
| 실험 | 내용 | 결과 | 판정 |
|---|---|---|---|
| W2 | 구축창 슬라이스 멀티스타트 (2× / 4×) | **+31% / +52% 악화** (7/7) | 기각 — 깊이 > 다양성 |
| W3 | 구축+LS 탐색폭 32/80→**48/120** | **−11.3%** (6/7 승) | **채택** |
| W4 | 구축/LS 분할 70/30→55/45·85/15 | +4.5% / +11.3% 악화 | 기각 — 70/30 유지 |
| S16 | safe 멀티스타트 8→**16 spec** | **−5.9%** (5승/4무) | **채택** (P3 보조 개선) |

**리스크 평가 (정직)**
- W3: 검사 로직·시간 메커니즘 불변(파라미터만) → 기존 wide와 동일 리스크 등급(서버 3회 실증).
  단, run-to-run 노이즈 ±15%로 end-to-end 확증은 −1.6%에 그침 — paired 6/7 승이 근거.
- S16: 인증 구조 그대로, 시간 초과시 자동 축소. P1/P2(narrow)·라우터·인증서·floor 무변경.
- 검증: 회귀 6/6 · placement PASS · 빌드 3경로 smoke(prob_22 5.02M→4.92M) ·
  **train 40 전수(서버-시뮬) 40/40 feasible**.

**기대**: P6/P5/P4 평균 ~5-10% 개선(분포상; 노이즈로 편차 가능), P3 ~3-5% 추가 개선.
총합 108.9M → **~100-105M** 기대(1차 목표 100M 근접).

## #10 — 2026-07-06 08:07:41 UTC — ✅ 6/6 feasible, P6 대폭 개선

| 인스턴스 | #9 | #10 | 변화 |
|---|---:|---:|---|
| P1 | 43,300 | **43,300** | narrow 불변 |
| P2 | 84,764 | **84,764** | narrow 불변 |
| P3 | 10,793,058 | **10,793,058** | safe 불변 |
| P4 | 10,761,704 | **10,467,216** | −2.7% |
| P5 | 25,903,553 | **23,265,756** | −10.2% |
| P6 | 61,359,812 | **47,152,499** | **−23.2%** (wide 48/120) |

총합 **91,806,593** (#9 108.9M 대비 −15.7%). wide 탐색폭 48/120이 서버에서 실증.

## ★ #11 준비분 — large route 안전 강화 (mp keep-min + LNS 보험)

**large route 병목 실험 (paired, 대형 train 8개, server-sim)**: 단일스레드 large 경로는
**단일 deep exit 구축 + 48/120 LS(70/30)** 가 측정된 국소최적. 대안 전부 열위/위험:
| 실험 | 결과 | 판정 |
|---|---|---|
| LNS를 개선단계 주력으로 | **+37%** 악화 | 기각 |
| 구축창 분할(2키/2시드) | +32~52% 악화 | 기각(구축 깊이 지배) |
| tard 키로 전환 | large −4.2% **그러나 인스턴스별 ±20% 동전던지기**(prob_26 −22% / prob_23 +21%) | **기각** — "개선원인 불명확 제출금지" 규칙 위반, 숨김 P4/P5 회귀 위험 |
| dual-key keep-min(35% 구축) | +32% 악화 | 기각(얕은 구축) |

**채택 (둘 다 non-regressing, 절대 악화 불가)**
1. **mp 워커 키 다양화**: `_worker` 4개를 `exit`+`tard`(×2시드)로 → 병렬이면 각자 full budget이라
   keep-min으로 per-instance 최적 키를 **공짜로** 획득. **단, mp가 서버에서 막혀 있으면(이력상 유력)
   이 블록은 예외 나고 단일스레드 경로로 그대로 낙하 → #10과 동일(무개선·무악화).**
2. **`_lns_phase` (LNS 꼬리)**: LS가 시간 내 수렴하면 남는 시간에 ruin-recreate(최다-tardy 파괴·
   재삽입), **strict 개선만 수락**. 현 실험 박스에선 LS가 30% 전부 소비해 no-op이나, 더 빠른
   서버에서 LS가 조기수렴하면 multi-block 이동으로 single-block 국소최적 탈출(무퇴보 보험).

**정직한 기대치**: 단일스레드 경로 **무변경** → 서버가 mp 차단이면 **#10과 동일(91.8M)**.
mp 허용이면 P4/P5/P6 keep-min 개선. 즉 **하방 리스크 0, 상방은 서버 mp 여부에 의존** +
**부수효과로 서버 mp 작동 여부를 확정**(다음 라운드 keep-min 멀티스타트 레버 판단 근거).

**검증**: 회귀 6/6 · placement PASS · 빌드 3경로 smoke feasible · narrow/safe 목적값 #10과
비트-동일(경로 무변경 확인).
