...
## #4 (예정) — 수정본(AABB fix + 안전망 + full-solver entrypoint) 재제출
제출 zip은 `myalgorithm.py`가 `solver.algorithm`을 호출하는지 빌드 검증으로 확인한다.
옛 버그본(37,169 bytes) 재사용 금지. 빌드: `python tools/build_submission.py`.
발송 후 결과 메일의 P1~P6를 아래 표에 기록할 것.

| 인스턴스 | #1 (기준) | #4 (수정본) | feasible | 비고 |
|---|---:|---:|:---:|---|
